...
public class DiagnosticoController : ControllerBase
{
    ...
    /// <summary>
    /// Obtener estructura configurable del formulario de diagnóstico
    /// </summary>
    [HttpGet("FormSections")]
    public async Task<List<FormularioSeccionResponse>> GetFormSections()
    {
        return await _mediator.Send(new GetFormularioSecciones());
    }

    /// <summary>
    /// Guardar estructura configurable del formulario de diagnóstico
    /// </summary>
    [HttpPut("FormSections")]
    public async Task<IActionResult> SaveFormSections([FromBody] GuardarFormularioSecciones command)
    {
        await _mediator.Send(command);
        return Ok("Se guardó la estructura del formulario correctamente");
    }
