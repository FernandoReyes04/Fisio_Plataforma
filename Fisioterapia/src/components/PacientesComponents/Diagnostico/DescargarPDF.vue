...
// Convierte base64 a Blob y devuelve una object URL.
// A diferencia de data URLs, las blob URLs no tienen límite de tamaño
// y funcionan correctamente en iframes para PDFs grandes.
...
// Liberar la blob URL al desmontar el componente para evitar memory leaks
onUnmounted(() => {
    if (blobUrl.value) {
...
const obtenerReporte = async () => {
    ...
    if (!response) {
        ...
        // Generar blob URL en lugar de data URL para evitar el límite de 2MB de Chrome
        blobUrl.value = base64ToBlobUrl(response.reporte)
    }
...
const descargarReporte = async () => {
    ...
    link.download = `reporte${props.nombre}.pdf`
...
<template>
    <div class="w-[1000px] bg-white p-4">
        ...
        <div class="w-full flex justify-end mb-2">
            <button
                class="bg-blue-600 px-4 py-2 font-poppins text-white rounded-[3px] flex gap-2 hover:bg-opacity-95 transition transition-duration-300"
                :class="{'button-disabled': loaderVistaPDF || botonEstado}"
                :disabled="loaderVistaPDF || botonEstado"
        ...
        <iframe
            ...
            class="h-[700px] w-full shadow-lg mb-2 border"
        ></iframe>
